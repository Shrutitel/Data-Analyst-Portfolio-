# T Shruthi - Data Analyst Portfolio

## About Me
Data Analyst with experience in SQL, Power BI, Python, Excel, and Tableau. Skilled in dashboard development, reporting automation, and business insights generation.

---

# Projects
## 1. Sales Dashboard - Power BI
### Tools Used
- Power BI
- SQL
- Excel

### Project Description
Built an interactive sales dashboard to analyze revenue trends, KPIs, customer performance, and regional sales insights.

---

## 2. Tableau Dashboard Project
### Tools Used
- Tableau
- Excel

### Project Description
Created interactive Tableau visualizations for trend analysis and business reporting.

---
# Skills
- Excel
- Statistics
- SQL
- Power BI
- Tableau
- ETL (Informatica)
- Python
- Cloud
- Reporting & Dashboard creation
- Data Analysing

---

# Resume 
[![Resume](https://github.com/Shrutitel/Data-Analyst-Portfolio-/blob/main/Tel_Shruthi_Resume-1%20(1).pdf)

---

# Certifications
- PL-300 Power BI Certification
- Microsoft Certified Azure (AZ-900)
- Google Cloud : Google Workspace Administrator 

---

# Contact
LinkedIn: www.linkedin.com/in/t-shruthi-b8a401250 
Email : shruthitel2017@gmail.com 
GitHub: (https://github.com/Shrutitel/Data-Analyst-Portfolio-/edit/main/README.md)
